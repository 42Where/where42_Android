...
class ListViewAdapter(val context: Context, val profileList: ArrayList<profile_list>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        val view: View = LayoutInflater.from(context).inflate(R.layout.list_view_detail, null)

        /* 위에서 생성된 view를 res-layout-main_lv_item.xml 파일의 각 View와 연결하는 과정이다. */
        val Photo = view.findViewById<ImageView>(R.id.profile_photo)
        val intraId = view.findViewById<TextView>(R.id.intra_id)
        val comment = view.findViewById<TextView>(R.id.Comment)
        val location = view.findViewById<TextView>(R.id.location_info)
